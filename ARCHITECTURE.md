...
только, кто кому звонит: он передаёт конверты сигналинга и выдаёт часовые
доступы к ретранслятору (`friends/calls` в trade-api). Своего медиасервера нет и
не планируется — при двух собеседниках он не нужен.

- [src/state/call.ts](src/state/call.ts) — состояние разговора и весь порядок
  действий: дозвон, ответ, обрыв, мут, глушение, показ экрана. Роль (звонящий /
  отвечающий) задаёт вежливость при пересогласовании и не меняется даже при
  пересборке соединения после обрыва.
...
- Показ экрана — `getDisplayMedia` с окном выбора самого движка. На macOS
  встроенный WKWebView его не отдаёт, кнопка там не показывается.
