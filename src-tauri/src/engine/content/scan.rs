...
/// Reads local metadata and identifies files by sha1 on Modrinth for content
/// that was not installed through the catalog (modpacks, manually added jars).
pub async fn scan_content(profile: String, kind: String) -> Result<ScanResult, String> {
    ...
    Ok(ScanResult { scanned, identified, items: list_content(&profile, &kind) })
}
